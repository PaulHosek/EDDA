...
```
The model found includes only "employ" and "lawyers". It explains a high proportion of the variance ($R^2=`r sprintf('%.3f',summary(best_model)$r.squared)`$). Analysis of collinearity found all VIF values larger than five. This model may not be the most suitable model as factors linearly depend on another.
...
summary(lass_model)
```
The model chosen by the LASSO method includes lawyers, employ, and pop as predictors. This model is more complex than the model chosen in b, as that model only contained the predictors of lawyers and employ. Interestingly, both models lead to a similar R-squared with $R^2=`r sprintf('%.3f',summary(lass_model)$r.squared)`$ (LASSO model) and $R^2=`r sprintf('%.3f',summary(best_model)$r.squared)`$ (step-down model), respectively. Given a similar model fitness, the more parsimonious model should be favored. The adjusted R-squared reflects this with $R^2_{adj} =`r sprintf('%.3f',summary(lass_model)$adj.r.squared)`$ (LASSO model) and $R^2_{adj} =`r sprintf('%.3f',summary(best_model)$adj.r.squared)`$ (step-down model). Therefore, the model based on the step-down approach should be slightly favored.
