...
First plotting the discrete distibution of values, we see that values apprear 
approximately normally distibuted. Further, QQ-plots indicate that samples 
from both measurements are similarly distibuted by insepection.
...
cat("Pvalues are", res1, "and", res2, "..")
```
Shapiro-Wilk tests for normality indicate that the assumption of normality
is met for both pre- and post-measurement. P-values are  0.9674667 for the 
...
```
# b)
First, we use a non-parameteric, repeated measures t-test.
```{r, echo=TRUE}
